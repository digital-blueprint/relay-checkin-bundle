...
class DbpLocationCheckInExtension extends ConfigurableExtension implements PrependExtensionInterface
{
    ...
    public function loadInternal(array $mergedConfig, ContainerBuilder $container)
    {
        ...
        $cacheDef = $container->register('dbp_api.cache.location_check_in.location', FilesystemAdapter::class);
        $cacheDef->setArguments(['location-check-in', 60, '%kernel.cache_dir%/dbp/location-check-in']);
        $cacheDef->setPublic(true);
        $cacheDef->addTag('cache.pool');

        $definition = $container->getDefinition('DBP\API\LocationCheckInBundle\Service\LocationCheckInApi');
